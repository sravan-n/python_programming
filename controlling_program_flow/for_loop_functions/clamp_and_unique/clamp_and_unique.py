...
def uniques(tup):
    ...
    # create an empty tuple acculator for unique nos
    unique_nos = ()
    # for each eleemeent x of tups
    for x in tup:
        # check if x is in the accuumulator
        if x in unique_nos:
            unique_nos = unique_nos
        else:
            # if x is not there in accumulatot then add it to acccumulator
            unique_nos = unique_nos + (x,)
    # compute the length of accumulator and assign it to result
    reult = len(unique_nos)
    # return result
    return reult
