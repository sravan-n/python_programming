...
def session(bet):
    ...
    while going:
        if choice == 'a':
            random_no = random.randint(4,7)
            score = score + random_no
            print("Your score is "+str(score)+".")
            if score > 20:
                going = False
                print('You busted.')
                payout_amt = payout(bet,score)
                if payout_amt >= 0:
                    print('You won '+str(payout_amt)+' credits.')
                else:
                    print('You lost '+str(-payout_amt)+' credits.')
                return payout_amt
            elif score == 20:
                going = False
                print('Quasar!')
...
def play(credits):
    ...
    while going:
        ...
        payout_amt = session(bet)
        credits = credits + payout_amt
        if credits == 0:
            going = False
